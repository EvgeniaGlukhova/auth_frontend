...
const canChangeStatus = computed(() => {
  ...
  return ['new', 'assembly', 'ready'].includes(props.order.status)
})
...
const getClientPhone = () => {
  if (!props.order) return 'Не указан'
  return props.order.client?.phone || props.order.client_phone || 'Не указан'
}
...
const getReadyDate = () => {
  ...
  const date = props.order.assembly_date || props.order.delivery_date
  const time = props.order.delivery_time || ''
  return date ? `${formatDate(date)} ${time}`.trim() : 'Не указана'
}
...
const getItemName = (item) => item.itemable?.name || 'Товар'
...
const getEmployeeName = () => {
  ...
  if (props.order.user) {
    const name = getUserFullName(props.order.user)
    if (name) return name
  }

  return props.order.user_id ? `ID: ${props.order.user_id}` : 'Не указан'
}

const getCourierName = () => {
  if (!props.order?.courier) return 'Не назначен'
  const name = getUserFullName(props.order.courier)
  return name || 'Курьер'
}

// кнопка
const getStatusAction = () => {
  if (!props.order) return { text: 'Изменить статус', class: 'default-btn', nextStatus: null }

  const actions = {
    new: { text: 'В работу', class: 'work-btn', nextStatus: 'assembly' },
    assembly: { text: 'Отправить в готовые', class: 'assemble-btn', nextStatus: 'ready' },
    ready: { text: 'Выдать заказ', class: 'complete-btn', nextStatus: null }
  }

  return actions[props.order.status] || { text: 'Изменить статус', class: 'default-btn', nextStatus: null }
}

const getStatusActionText = () => getStatusAction().text
const getStatusActionClass = () => getStatusAction().class
...
const changeStatus = () => {
  ...
  // Для статуса ready - вызываем завершение со списанием
